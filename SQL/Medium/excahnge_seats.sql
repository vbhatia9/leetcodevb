...
/* Given that the task specifies dealing with odd and even IDs, 
we can efficiently separate these scenarios using a CASE WHEN clause.
...
For records with an odd ID, swap the names with the subsequent record. 
Use the window function LEAD(student) OVER (ORDER BY id) to fetch the name from the line below.
